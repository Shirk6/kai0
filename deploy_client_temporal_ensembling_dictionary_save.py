...
class InferController:
    def __init__(self, host="localhost", port=8000):
        ...
        # === 指令 ===
        # self.instruction = "pack objects on the table into the container" # 这个指令不重要，主要是为了让模型输出动作时能对齐训练时的 prompt，实际执行时模型也不一定完全按照指令来
        # self.instruction = "stack three cups into a tower"
        self.instruction = "Insert the battery into the mouse."
        ...
        # pack_3_objects_plus
        # self.LEFT_INIT_POSITION = [-0.5185, 1.2758, -0.8406, 0.0315, 1.0872, 0.1147, 0.0593]
        # self.RIGHT_INIT_POSITION = [0.4801, 1.1595, -0.7810, 0.0920, 1.1126, -0.0512, 0.0515]
        # stack_3_cups
        # self.LEFT_INIT_POSITION = [-0.75764, 1.03441, -0.74778, 0.03059, 0.93736, -0.07892, 0.06808]
        # self.RIGHT_INIT_POSITION = [0.63639, 1.00319, -0.71547, 0.01034, 1.02991, 0.19950, 0.06644]
        # battery_assemble
        self.LEFT_INIT_POSITION = [-0.4582, 1.2142, -0.8490, 0.0574, 0.9980, 0.1302, 0.0650]
        self.RIGHT_INIT_POSITION = [0.4609, 1.2258, -0.9031, 0.1171, 1.0770, -0.1122, 0.0560] 
    ...
    def _action_producer_loop(self, ros_operator):
        ...
        while not _action_stop_event.is_set() and not rospy.is_shutdown() and not stop_signal.is_set():
            ...
            # }
            obs={
                "images": {"top_head": img_h_processed, 
                            "hand_left": img_l_processed, 
                            "hand_right": img_r_processed},
                "state": self.get_status_and_state(),
                "prompt": self.instruction
            }        
...
def get_arguments():
    ...
    parser.add_argument('--output_dir', action='store', type=str, 
                        default='/home/agilex/World_Action_Model/cobot_magic/collect_data/data/pack_objects_infer/pi05-pretrain', required=False)
